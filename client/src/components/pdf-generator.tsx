...
export default function PDFGenerator({ formData, isFormValid, onValidate }: PDFGeneratorProps) {
  ...
  const generatePDF = async (): Promise<void> => {
    ...
    try {
      ...
      doc.text('FLIGHT BOOKING CONFIRMATION', margin, 28);
      ...
      doc.rect(margin - 5, yPos - 8, pageWidth - (margin * 2) + 10, 25, 'F');
      ...
      yPos = addText(`Phone Number: ${formData.phone}`, margin + 5, yPos, 12);
      yPos += 15;
      ...
      formData.flightSegments.forEach((segment, index) => {
        ...
        doc.setTextColor(0, 0, 0);
        yPos = addText(`Flight Segment ${index + 1}`, margin + 25, yPos, 14, 'bold');
        ...
        yPos = addText(`Departure: ${segment.departureTime} | Arrival: ${segment.arrivalTime}`, margin + 10, yPos, 12);
        yPos += 10;
      });
      ...
      formData.passengers.forEach((passenger, index) => {
        ...
        doc.setTextColor(0, 0, 0);
        yPos = addText(`Passenger ${index + 1}`, margin + 25, yPos, 14, 'bold');
        yPos = addText(`Name: ${passenger.name}`, margin + 10, yPos, 12);
      ...
      if (formData.paymentMethod) {
        ...
        yPos = addText(`Payment Method: ${paymentText}`, margin + 5, yPos, 12);
        yPos += 15;
      }
      ...
      yPos = addText('TERMS & CONDITIONS', margin, yPos, 16, 'bold');
      yPos += 8;
      yPos = addText('✓ Customer has agreed to all booking terms and conditions', margin + 5, yPos, 12);
      yPos += 20;
